...
|---|---|---|---|
| 2026-09-06 | working tree | **1.59.4** — the same change as 1.59.3, tagged on the verified merge commit. The v1.59.3 tag was pushed against `origin/main` before the release PR had merged — it names the v1.59.2 tree, the version-sync gate refuses it, and the remote forbids tag deletion, so the version is burned with nothing shipped under it. **Diverged once, and it is the orchestrator's:** the tag was cut from "whatever origin/main is" instead of from a commit whose subject names the merged PR — the check that stops the recurrence is reading the tip's subject for the PR number before tagging. | yes — see left |
| 2026-09-06 | working tree | **1.59.3** — the front matter stops declaring only the optional half of its dependencies, and the mirror gate learns to see directories. `compatibility:` named three skills the body works without and said nothing about node+npx+network, which the body requires twice; it now opens with the requirement, and the kit-authoritative paragraph states the degraded path — packaged pack and token layer still read, kit-materialized states are then unverified and say so. The audit's second finding was an empty recursive `.cursor/skills/…/styles/tokens/.cursor/skills/…` chain sitting in checkouts: zero files, invisible to git, to npm and to both `is_file()` walks of the mirror check. The class is closed — directories are read over both trees, nested `.cursor` and empty dirs refused — watched failing on a real planted chain with the checks' own messages, then planted permanently as two directory fixtures the text-mutation `PLANTS` table cannot express, each asserting it landed before trusting a red. **Diverged twice.** (1) The honest 342-char compatibility draft was inside the spec's 500 cap and REFUSED by this repo's own 256 overhead budget at 432 — the binding limit was the house budget, not the spec cap, the same two-limits lesson v1.59.2 paid for on the description; the compressed form ships at 253 of 256 and the body carries the detail. (2) The planted artefact was not in this repository's checkout at all — measured clean here — it sat in the umbrella's submodule checkout at `skills/sheleg-design` (commit `0902ef6`); removed there, and the gate now refuses the class wherever the checkout lives. | yes — see left |
| 2026-09-03 | working tree | **1.59.2** — the skill now advertises the two words its own router text names. The umbrella's block says *before design, redesign, layout…* and the trigger layer fires only on words a skill claims about itself, so `редизайн сайта`, `переделай дизайн` and `свёрстай дашборд` all reached nothing while the paragraph telling an agent what to do about them shipped. Final: **960 of 970**, bought by trimming prose. **Diverged three times, and all three are one shape — I proved something safe with an instrument that could not see the other consumer.** (1) I declared the 970 limit non-existent because this repository's `test/validate.py` enforces only the Agent Skills cap of 1024, wrote that in the CHANGELOG and in this table, and shipped a description at 1014. **CI refused it**: 970 is a family HOUSE RULE in `make-skill`'s `audit_skill.py --house`, running in this repository's own workflow. Two limits, two enforcers, one of them in another repository. Both false sentences were rewritten rather than deleted. (2) The first trim cut `particle`, `scrubbed` and `drift` after checking them against `lib/triggers.js` and finding no hook trigger depended on them — they are `T1_CARRIERS` here, the phrases this repository's eval tasks match on, and its own gate refused the edit. The shipping trim was tested against BOTH lists, by a script rather than by reading, precisely because two partial checks had already been wrong. (3) **Instruction #6 fired on the guard protecting (2).** Its plant hardcoded `scroll-linked or scrubbed motion`; the trim reworded that clause, the replace no-oped, and the self-test reported a healthy guard as BROKEN. Re-anchored on `T1_CARRIERS` with the assertion made on the parsed description, read the way the guard reads it. | yes — see left |
| 2026-09-03 | working tree | **1.59.1** — the director's Act 2 gains the fallback this skill's own front matter promises for every optional sibling. It mandated `npx sshlg-skills pack design --lane <lane>` with no path for a machine that does not have the umbrella, while `shadcn`'s and `dataviz`'s siblings both say what happens when they are absent. The replacement is not a shrug: the lane table is in the document and the harness lists installed skills, so two of the command's three answers survive by reading — what is MISSING and what was DECLINED do not, and the cast must say the roster was unmeasured. **Diverged: found by the close-out ladder walk, not by any gate.** The validator holds the companion, its link and its derived contents; nothing checks a sibling's fallback, because that convention is a claim the skill makes about itself in its own front matter. A comparison needs two sides and this had one. | yes — see left |
| 2026-09-03 | working tree | **1.59.0** — the skill gains a decision layer above its craft references. `CREATIVE_DIRECTOR.md` is now the first read: a brief whose fourth line is a falsifier, a mode table where a redesign enters by measuring what it replaces, a cast chosen with the umbrella's new `pack design --lane` rather than from memory, a two-subagent fork under a rubric written **before** either variation exists, and a validation split that separates alignment from eight measurable quality checks and then states plainly that taste is on neither list. Held by the validator as a companion, so its absence cannot be silent — the craft references read fine without it, which is the reason it needs a mechanical holder. **Diverged: three, all caught by this repository's own gate rather than by reading** — the bundle file list in `install.sh` did not carry the new file, the derived `## Contents` list was missing, and the draft cited `docs/ux/scenarios.md`, a repository path that dead-ends for anyone who installed the bundle instead of cloning it. That third rule is the one worth naming: a reference can be perfectly correct in the repo and useless in the artefact, and only a check that knows the difference finds it. | yes — see left |
