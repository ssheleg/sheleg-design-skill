...
| P-16 | The routing scenario and its negative branch have **run** | T29a / T29b in fresh contexts | **never** — written in this change, not executed. Recorded as unrun in `test/scenarios.md`, and it matters more than it did for T28: `ora` sits inside the same silhouette, so T29a has two packs to reject | **never** |
