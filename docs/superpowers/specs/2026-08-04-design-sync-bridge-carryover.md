...
| 21 | 10 Acceptance | `SKILL.md` carries no version in its front-matter, so the installed skill cannot say which version it is — and the bridge's own §7 says "the pack's version is the design system's version" | found by the T14 scenario agent applying this run's own rule against this run's own docs. Adding a `version:` key touches the description canon and the four-way sync, which is a repo-wide contract change, not a bridge change | REQ-010 | **backlog** — decide with the doc regime; the rule is currently uncheckable |
| 22 | 10 Acceptance | The CI matrix listed six packs by hand, so the seventh kit was built, green and invisible to CI | fixed same-stage by deriving the matrix from `kits/` — a hand-maintained list is the same defect class the kit check catches, one layer up | REQ-016 | resolved — `.github/workflows/validate.yml` |

## Counts

`open: 19 · unresolved: 0 · dropped: 0` — of which **open and owned**: rows 2, 3, 4, 5, 7, 12, 13, 16, 19, 21. Rows 9, 15, 17, 20, 22 resolved in-stage; rows 1, 6, 8, 10, 11, 14, 18 closed by the release or the merge. (rows 9, 15 and 17 resolved same-stage) — printed beside every gate verdict from here on.
