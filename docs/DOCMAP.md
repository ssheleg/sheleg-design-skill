...
| **A long reference's `## Contents` list** | the file's own `## ` headings — the list is derived, never authored | `validate_contents_lists()` imports the derivation from `scripts/gen_contents.py`; `npm run gen-contents` regenerates bundle, mirror and template source together (SHD-01) |
| **The two host-facing manifest descriptions** (`plugin.json`, the plugin entry in `marketplace.json`) | `STYLE_PACK_INDEX.md`'s catalogue table — register and good-fit columns | `validate_manifest_descriptions()` imports the derivation from `scripts/gen_manifest_descriptions.py`; `npm run gen-descriptions` regenerates. Hand-appending per release is what glossed `patchbay` and `nameplate` twice and `deskmate` never (SHD-03) |
...
| **New style pack** | `styles/<pack>.md` with **all thirteen headings** and a `Contract:` line (plus `## Motion flavor` for a cinematic pack) · `styles/tokens/<pack>.css` · the `.cursor/` mirror · **`install.sh`'s file list** · route from the `SKILL.md` pack table · a catalogue row in `STYLE_PACK_INDEX.md` · `npm run gen-contents` (the derived `## Contents` list) · `npm run gen-descriptions` (both manifest descriptions re-derive from the catalogue row) · name it in `bin/cli.js` output · README pack table · name it in a `cursor/rules/*.mdc` · **a kit under `kits/<pack>/`** · a routing scenario **with its negative branch** · **the kit rendered in a browser at three widths, its computed values checked against the pack's claims** · CHANGELOG | `python3 test/validate.py`, and for the render `getComputedStyle` read back at 1440 / 768 / 390 — a gate reads structure, not layout, and two consecutive releases shipped a layout defect over three green gates |
...
thirty-nine-kit matrix today. The number is never typed into the workflow:
