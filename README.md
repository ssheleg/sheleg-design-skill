...
decides whether to animate before it decides how, and **eighteen locked style
...
| `styles/*.md` | The eighteen style packs — palette, type, texture, motion tokens, motifs, bans, and the traps each one carries |
...
identical in all eighteen kits, so switching packs swaps identity rather than API,
...
installers and `diff -r`s the result against the source, then builds all eighteen
kits.
