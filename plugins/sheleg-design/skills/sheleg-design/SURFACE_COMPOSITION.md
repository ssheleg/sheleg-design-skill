...
the thirty-nine packs: only `--bg` and `--ink` resolve in every pack. The accent is
`--accent` in thirty-five of the thirty-nine packs; the other four name it themselves —
`--brand` in `babylove` and `field-notes`, `--cta` in `orchard`, `--coral` in `chorus` —
and each declares
...
`@role non-text:` with the reason and the ratio, because twenty-six of the token layers
...
| `--good` / `--warn` | babylove |
| `--danger` / `--good` / `--info` | atrium, chorus, surveyor |
| `--danger` / `--good` | roster |
| `--danger` | field-notes |
| `--good` | briefing-room |
