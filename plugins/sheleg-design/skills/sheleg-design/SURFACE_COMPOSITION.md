...
the fifteen: only `--bg` and `--ink` resolve in every pack. The accent is
`--accent` in thirteen, `--brand` in `field-notes` and `--cta` in `orchard` (each
...
| Surfaces | `--bg` for light, the pack's dark field for dark | resolves in all fifteen |
