...
the twenty: only `--bg` and `--ink` resolve in every pack. The accent is
...
`@role non-text:` with the reason and the ratio, because fourteen of the twenty packs
...
| Surfaces | `--bg` for light, the pack's dark field for dark | resolves in all twenty |
