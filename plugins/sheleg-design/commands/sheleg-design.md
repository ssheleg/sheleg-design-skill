...
- **If the arguments name a style pack, use that pack without re-asking.** The
  ...
  `blueprint`, `prism`, `maquette`, `scoreboard`, `datasheet`, `manpage`, `pigeonhole`, `roster`, `ora`, `tenor`, `paperclip`, `ledger`, `awning`, `router`, `daylight`, `notation`, `almanac`, `vitrine`, `proscenium`. Otherwise pick per the `SKILL.md` table,
