...
the thirty-five packs: only `--bg` and `--ink` resolve in every pack. The accent is
`--accent` in twenty-eight of the thirty-five packs, `--brand` in `field-notes` and `--cta` in `orchard` (each
...
| `--danger` / `--good` / `--info` / `--warn` | bulletin, nameplate, onionskin, ora, paperclip, pigeonhole, rimlight, scoreboard |
