...
the thirty-two packs: only `--bg` and `--ink` resolve in every pack. The accent is
`--accent` in twenty-eight of the thirty-two packs, `--brand` in `field-notes` and `--cta` in `orchard` (each
