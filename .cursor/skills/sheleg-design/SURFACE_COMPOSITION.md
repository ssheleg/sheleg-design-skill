...
the nineteen: only `--bg` and `--ink` resolve in every pack. The accent is
`--accent` in seventeen, `--brand` in `field-notes` and `--cta` in `orchard` (each
...
`@role non-text:` with the reason and the ratio, because thirteen of the nineteen packs
...
| Surfaces | `--bg` for light, the pack's dark field for dark | resolves in all nineteen |
