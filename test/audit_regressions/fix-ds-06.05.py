#!/usr/bin/env python3
"""FIX-DS-06.05 — the large-text visual-composition eval corpus
(sherlock audit, DS-06/XD-13).

The corpus (evals/cases/fix-ds-06.05.json) separates deterministic
...
CASES = os.path.join(ROOT, "evals", "cases", "fix-ds-06.05.json")
...
def t_required_case_shapes_present():
    ...
    for shape in ("identity-preserving", "zoom-pressure", "hierarchy-survives-zoom",
                  "no-screenshot", "scope-honesty"):
...
def t_identity_preserving_and_faults():
    ...
    mp = next(c for c in m["cases"] if "zoom-pressure" in c["id"])
    ...
    rb = next(c for c in m["cases"] if "hierarchy" in c["id"])
