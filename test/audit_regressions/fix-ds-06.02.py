#!/usr/bin/env python3
"""FIX-DS-06.02 — the consumer-chat visual-composition eval corpus
(sherlock audit, DS-06/XD-13).

The corpus (evals/cases/fix-ds-06.02.json) separates deterministic
...
CASES = os.path.join(ROOT, "evals", "cases", "fix-ds-06.02.json")
...
def t_required_case_shapes_present():
    ...
    for shape in ("identity-preserving", "message-pressure", "input-bar-reachability",
                  "no-screenshot", "dark-theme-parity"):
...
def t_identity_preserving_and_faults():
    ...
    mp = next(c for c in m["cases"] if "message-pressure" in c["id"])
    ...
    rb = next(c for c in m["cases"] if "reachability" in c["id"])
