#!/usr/bin/env python3
"""FIX-DS-06.03 — the mobile-sheet visual-composition eval corpus
(sherlock audit, DS-06/XD-13).

The corpus (evals/cases/fix-ds-06.03.json) separates deterministic
...
CASES = os.path.join(ROOT, "evals", "cases", "fix-ds-06.03.json")
...
def t_required_case_shapes_present():
    ...
    for shape in ("identity-preserving", "height-and-content-pressure", "dismiss-affordances",
...
def t_identity_preserving_and_faults():
    ...
    rb = next(c for c in m["cases"] if "dismiss" in c["id"])
