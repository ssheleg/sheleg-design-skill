...
export function Stat({ value, label, source, className }: StatProps) {
  return (
    <div className={['pc-stat', className].filter(Boolean).join(' ')}>
      <span className="pc-stat__label">{label}</span>
      <span className="pc-stat__value">{value}</span>
      {source !== undefined && <span className="pc-stat__source">{source}</span>}
    </div>
  );
}
