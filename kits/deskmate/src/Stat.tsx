...
export function Stat({ value, label, source, className }: StatProps) {
  return (
    <div className={['dm-stat', className].filter(Boolean).join(' ')}>
      <span className="dm-stat__label">{label}</span>
      <span className="dm-stat__value">{value}</span>
      {source !== undefined && <span className="dm-stat__source">{source}</span>}
    </div>
  );
}
